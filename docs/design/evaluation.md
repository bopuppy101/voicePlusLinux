...
The [admission reference](../../experiments/contract_reference/README.md) covers a limited subset of G1; the complete production contract gate remains pending. The [intent development set and scorer](../../experiments/intent_evaluation/README.md) provide public synthetic text cases, including bounded correction context, not held-out model results. The [manifest template](benchmark-manifest.template.json) records required benchmark inputs without inventing values. Current software-check counts are recorded in [progress](progress.md). Real audio, model predictions, and performance measurements are still absent.
