...
## External microphone choices and setup

**A display without far-field microphones remains a valid candidate.** An external microphone connects directly to the existing Dell running Ubuntu. A sofa-table placement nearer Mike is acceptable; distinguish mouth-to-microphone distance from screen distance.

The single comprehensive hardware comparison, including ReSpeaker XVF3800, Jabra Speak2 75, Yamaha YVC-1000, prices, advertised ranges, processing, speaker output, connections, Ubuntu status, and setup steps, is maintained in:

[/home/mike/git/voicePlusLinux/docs/OS-Integration-Design/visual-and-audio-user-interface-options.md](../OS-Integration-Design/visual-and-audio-user-interface-options.md#hardware-shortlist-discussed-with-mike--september-19-2026)

ReSpeaker is the recommended first capture trial, not a measured performance winner or an approved fully open processing stack. Stem Wall is excluded from new-purchase recommendations because Shure marks it discontinued. Keep product comparisons in that one table; this document covers the acoustic problem and evaluation method.
